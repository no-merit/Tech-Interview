...
<li> Big-O, Big-Theta, Big-Omega 에 대해 설명해 주세요.</li>
  ...
  Big-O      :최악의 경우, 점근적 상한,n이 증가함에 따라 f(n)이 O(n^2)보다 클 수 없음.
  Big-Theta  :O와 Omega 표기가 같은 경우에 사용. f(n)은 n이 증가함에 따라 n^2과 동일한 증가율을 가진다.
  Big-Omega  :최선의 경우, 점근적 하한,n이 증가함에 따라 f(n)이 O(n^2)보다 작을 수 없음.
...
<li> 다른 것을 사용하지 않고, Big-O를 사용하는 이유가 있을까요?</li>
  ...
  Big-O는 점근적 상한을 나타내는 표기법으로 해당 알고리즘의 최악의 경우를 나타냄.
  알고리즘의 평균적인 시간은 의미가 없는 경우가 많기 때문
  이정도 시간이 걸릴거야하고 말하는 것보다 절대 이 시간 이상은 넘지 않는다고 말하는 것이 신뢰도가 높음.
  
<li> O(1)은 O(N^2) 보다 무조건적으로 빠른가요?</li>
  복잡도를 계산할 때는 입력 크기 N이 무한대로 향한다고 생각하기 때문에 상수를 무시함
  O(3*N) = O(N), Ω(10logN) = Ω(logN)
  마찬가지로 O(1000)은 O(1)로 표기한다. 
  동시에 N<1000 이면, O(1)은 O(N) 보다 느리다.
</ul>
</details>
